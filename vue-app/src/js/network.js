...
export default class Network {
  ...
  simulate(event) {
    ...
    const simulation = d3
      .forceSimulation()
      .force("y", d3.forceX(width / 2).strength(0.4))
      .force("x", d3.forceY(height / 2).strength(0.6))
      .force(
        "charge",
        d3.forceManyBody().strength(forceProperties.chargeStrength)
      )
      .force(
        "link",
  ...
  setSize() {
    this.svg.attr("width", this.chartWidth).attr("height", this.chartHeight);
  ...
  initZoom() {
    ...
    const _this = this;
    function zoomed() {
      _this.chartLayer.attr("transform", d3.event.transform);
    }

    const g_buttons = this.svg
      .append("g")
      .attr("id", "zoom_buttons")
      .attr("transform", "translate(8, 8)");

    const reset = g_buttons
      .append("rect")
      .attr("width", "1em")
      .attr("height", "1em")
      .text("reset")
  ...
  draw() {
    ...
    // link update
    this.select.linkPolygons = this.chartLayer
      .selectAll(".linkPolygon")
      .data(this.current.links, linkKey);

    // node update
    this.select.nodeCircles = this.chartLayer
